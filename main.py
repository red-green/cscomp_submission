...
if not os.path.exists(DATABASE):
	...
	c = conn.cursor()
	c.execute('CREATE TABLE programs (hash text, team text, problem text, code text, ts int)')
	conn.commit()
	conn.close()
...
def submit():
	...
	conn = sqlite3.connect(DATABASE)
	c = conn.cursor()
	c.execute('INSERT INTO programs VALUES (?,?,?,?,?)',(hashlib.md5(program).hexdigest(),team,problem,program,time.time()))
	conn.commit()
	conn.close()
...
def view(cs):
	...
	i = c.fetchone()
	conn.close()
	a = {'hash':i[0],'team':i[1],'problem':i[2],'time':time.strftime('%I:%M:%S %p',time.gmtime(i[4])),'code':i[3]}
...
def submissions():
	...
	table = []
	for i in c.fetchall():
		table.append({'hash':i[0],'team':i[1],'problem':i[2],'time':time.strftime('%I:%M:%S %p',time.gmtime(i[4]))})
	conn.close()
